...
const FAQSection = () => {
  ...
  const [heading, setHeading] = useState("Pertanyaan yang Sering Diajukan");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (mounted && data?.length) setFaqs(data.map((f) => ({ q: f.question, a: f.answer })));
      const { data: cfg } = await supabase.from("faq_section_settings").select("*").limit(1).maybeSingle();
      if (mounted && cfg?.heading) setHeading(cfg.heading);
    };
    load();
    return () => {
      mounted = false;
